            ...
            <div id="container-avisos">
                ...
                <c:if test="${not empty sessionScope.mensagemSucesso}">
                    <div class="alerta-custom alerta-sucesso">
                        ...
                        <button type="button" class="btn-fechar-alerta" onclick="this.parentElement.style.display='none'"></button>
                    </div>
                    <% session.removeAttribute("mensagemSucesso"); %>
                </c:if>
            </div>
                ...
                <div class="table-header-row">
                    <div class="table-title">Perfis Cadastrados</div>
                     <div class="header-actions" style="display: flex; flex-direction: row; align-items: center; gap: 10px;">
                    	<button class="btn-report" onclick="window.location.href='${pageContext.request.contextPath}/RelatorioPerfilController'">
                                ...
                                <c:otherwise>
                                    <c:forEach var="p" items="${perfis}">
                                        ...
                                        <c:set var="isProtegido"
                                               value="${p.nome eq 'ADMINISTRADOR' or p.nome eq 'VISITANTE'}" />
                                        <tr>
                                            <td>${p.id}</td>
                                            <td>
                                                <span class="badge-perfil">${p.nome}</span>
                                                ...
                                                <c:choose>
                                                    ...
                                                    <c:otherwise>
                                                        ...
                                                        <button class="btn-icon" title="Editar"
                                                            onclick="openModalEditar('${p.id}', '${p.nome}')">
                                                            <i class="fa-solid fa-pen"></i>
                                                        </button>
            ...
            <div class="modal-body">
                <form id="formPerfil" action="${pageContext.request.contextPath}/PerfilController" method="post" autocomplete="off">
                    ...
                    <div class="form-group full-width">
                        ...
                        <input type="text" name="nome" class="form-input" placeholder="Ex: ADMINISTRADOR, OPERADOR, TÉCNICO" oninput="this.value = this.value.toUpperCase()" required>
                    </div>
                </form>
            </div>
        ...
        <div class="modal-box">
            ...
            <div class="modal-body">
                <form id="formEditarPerfil" action="${pageContext.request.contextPath}/PerfilController" method="post" autocomplete="off">
                    ...
                    <input type="hidden" name="id" id="edit_id_perfil">

                    <div class="form-group full-width">
                        <label class="form-label">Nome do Perfil</label>
                        <input type="text" name="nome" id="edit_nome_perfil"class="form-input" oninput="this.value = this.value.toUpperCase()" required>
                    </div>
                </form>
            </div>
    ...
    <script>
        ...
        function excluirPerfil(id) {
            if (confirm('Tem certeza que deseja excluir este perfil?\nAtenção: perfis vinculados a usuários não poderão ser excluídos.')) {
                ...
                idInput.type = 'hidden'; idInput.name = 'idperfil'; idInput.value = id;
